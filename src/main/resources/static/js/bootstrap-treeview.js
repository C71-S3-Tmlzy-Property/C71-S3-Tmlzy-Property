...
;(function ($, window, document, undefined) {
	...
	var Tree = function (element, options) {
		...
		return {
			...
			filterChecked: $.proxy(this.filterChecked, this),
	...
	*/
	...
	Tree.prototype.filterChecked = function (pattern, attribute, modifier) {
		attribute = attribute || 'text';
		modifier = modifier || 'g';
		// return this._orderedNodes;
		return $.grep(this._orderedNodes, $.proxy(function (node) {
			if (node.$el.attr('class').indexOf('node-checked') > -1) {
				return node;
			}
		}, this));
	};
